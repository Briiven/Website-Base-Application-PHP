					...
					<div class="pb-20">
						...
						<table class="data-table table stripe hover nowrap">
							...
							<tbody>
								...
								$sql = "SELECT application.id as lid, durationID, applicants.firstName,applicants.lastName,applicants.location,applicants.appID,applicants.program,application.appType,application.appDate,application.RAstatus,application.CRstatus,application.DHstatus,application.CDstatus from application join applicants on application.appID=applicants.appID 
								WHERE durationID = $session_duration_id";

								// Apply program filter
								if (isset($_GET['filterProgram']) && !empty($_GET['filterProgram'])) {
									$filterProgram = $_GET['filterProgram'];
									$sql .= " AND applicants.program = '$filterProgram'";
								}

								// Apply application type filter
								if (isset($_GET['filterAppType']) && !empty($_GET['filterAppType'])) {
									$filterAppType = $_GET['filterAppType'];
