...
<?php
$query = mysqli_query($conn, "select * from application where appID = '$session_id'") or
    die(mysqli_error($conn));
$row = mysqli_fetch_array($query);
$RAstaffID = $row['RAstaffID'];

// Get the program and department of the applicant
$query2 = mysqli_query($conn, "
            SELECT d.departmentShortName 
            FROM applicants a
            JOIN programs p ON a.program = p.ProgramShortName
            JOIN departments d ON p.deptID = d.id
            WHERE a.appID = '$session_id'
        ") or die(mysqli_error($conn));

$datarow = mysqli_fetch_array($query2);
$applicant_department = $datarow['departmentShortName'];
...
$dompdf->render();
$dompdf->stream("pdf.php", array("Attachment" => 0));
